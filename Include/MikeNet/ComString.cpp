...
void ComString::Allocate( size_t length )
{
	...
	comString = SysAllocStringLen(NULL,length);
...
void ComString::Allocate( const char * str )
{
	...
	if(length > 0)
	{
		int iResult = MultiByteToWideChar(CP_ACP,MB_COMPOSITE,str,length,comString,length);
...
void ComString::CopyIntoPacket( Packet & packet ) const
{
	...
	packet.Enter();
		...
		if(length > 0)
		{
			int result = WideCharToMultiByte(CP_ACP,WC_COMPOSITECHECK,comString,length,packetDataPtr,length,NULL,NULL);
