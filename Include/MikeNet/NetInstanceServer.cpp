 ...
 */
void NetInstanceServer::Initialize(size_t p_maxClients, bool p_handshakeEnabled, size_t p_connectionTimeout, NetSocketListening * p_socketListening)
{
...
{
	_ErrorException((clientID > maxClients),"performing a client related function on the server side. Invalid client ID", 0, line, file);
}
...
void NetInstanceServer::DisconnectClient(size_t clientID)
{
	...
	if(client[clientID].GetConnectionState() == NetUtility::CONNECTED ||
		client[clientID].GetConnectionState() == NetUtility::DISCONNECTING)
	{
	...
	client[clientID].SetConnectionState(NetUtility::NOT_CONNECTED);
}
...
bool NetInstanceServer::TestClass()
{
	...
	NetUtility::LoadEverything(2,0);
	{
		...
		Timer sendFreqTCP(100);
		Timer sendFreqUDP(100);
		Timer clientDisconnectFreq(200);
		Timer clientReconnectFreq(200);
		...
		while(loopRunTime.GetState() == false)
		{
			...
			if(newClient > 0)
			{
				cout << "A new client has joined: " << newClient << "\n";
			}
			...
			for(size_t n = 1;n<server->GetMaxClients();n++)
			{
				if(server->ClientConnected(n) == NetUtility::CONNECTED)
				{
					if(server->GetPacketFromStoreTCP(&recvPacket,n) > 0)
					{
						...
						cout << "TCP packet received by server from client " << recvPacket.GetClientFrom() << ": " << clock << ", " << str << '\n';
						delete[] str;
					}
					...
					if(server->GetPacketFromStoreUDP(&recvPacket,n) > 0)
					{
						clock_t clock = recvPacket.GetClockT();
						char * str = recvPacket.GetStringC(0,true);
						cout << "UDP packet received by server from client " << recvPacket.GetClientFrom() << ": " << clock << " with age of " << recvPacket.GetAge() << '\n';
						delete[] str;
					}
				}
			}
				...
				if(client.IsAllocated(n))
				{
					if(client[n].ClientConnected() == NetUtility::CONNECTED)
					{
						...
						if(client[n].GetPacketFromStoreTCP(&recvPacket) > 0)
						{
							clock_t clock = recvPacket.GetClockT();
							char * str = recvPacket.GetStringC(0,true);
							cout << "TCP packet received by client " << client[n].GetClientID() << ": " << clock << ", " << str << '\n';
							delete[] str;
						}
						...
						if(client[n].GetPacketFromStoreUDP(&recvPacket) > 0)
						{
							clock_t clock = recvPacket.GetClockT();
							char * str = recvPacket.GetStringC(0,true);
							cout << "UDP packet received by client " << client[n].GetClientID() << ": " << clock << ", " << str << " with age of " << recvPacket.GetAge() << '\n';
							delete[] str;
						}
					}
					else
...
*/
void NetInstanceServer::SetAffixEnableTCP(size_t clientID, bool enabledflag)
{
	ValidateClientID(clientID,__LINE__,__FILE__);
	NetInstanceTCP::SetAffixEnableTCP(clientID,enabledflag);
}
...
* @param clientID ID of client to use, may be ignored.
*/
bool NetInstanceServer::GetAffixEnableTCP(size_t clientID)
{
	ValidateClientID(clientID,__LINE__,__FILE__);
	return(NetInstanceTCP::GetAffixEnableTCP(clientID));
}


/**
* @brief Techlord Modification.
*/
void NetInstanceServer::SetBinaryModeTCP(size_t clientID, bool enabledflag)
{
	ValidateClientID(clientID,__LINE__,__FILE__);
	NetInstanceTCP::SetBinaryModeTCP(clientID,enabledflag);
}
...
* @param clientID ID of client to use, may be ignored.
*/
bool NetInstanceServer::GetBinaryModeTCP(size_t clientID)
{
	ValidateClientID(clientID,__LINE__,__FILE__);
	return(NetInstanceTCP::GetBinaryModeTCP(clientID));
}
...
* @brief Techlord Modification.
*/
void NetInstanceServer::SetHeaderFoundTCP(size_t clientID, bool foundflag)
{
	ValidateClientID(clientID,__LINE__,__FILE__);
	NetInstanceTCP::SetHeaderFoundTCP(clientID,foundflag);
}
...
* @param clientID ID of client to use, may be ignored.
*/
bool NetInstanceServer::GetHeaderFoundTCP(size_t clientID)
{
	ValidateClientID(clientID,__LINE__,__FILE__);
	return(NetInstanceTCP::GetHeaderFoundTCP(clientID));
}