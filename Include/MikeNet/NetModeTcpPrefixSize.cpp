...
void NetModeTcpPrefixSize::DealWithData(const WSABUF & buffer, size_t completionBytes, NetSocket::RecvFunc tcpRecvFunc, size_t clientID, size_t instanceID)
{
	...
	try
	{
		...
		// All characters up to partialPacket's cursor will be erased because they have been moved into the
		...
		// Set cursor to 0, we don't yet know what needs to be erased
		// MUST be done before ReCheck tag
...
ReCheck:
		...
		if(unreadData >= Packet::prefixSizeBytes)
		{
			...
			if(packetSizeWithPrefix > partialPacket.GetMemorySize())
			{
				if(autoResize.Get() == false)
				{
				...
				{
					// Increase memory size to accommodate incoming packet
					partialPacket.ChangeMemorySize(packetSizeWithPrefix);
				}
			}
			...
			if(packetSizeWithPrefix <= unreadData)
			{
				...
				copyData.len = packetSize;

				// Copy data from partialPacket into a separate packet to be passed to user
				Packet * completePacket = new (nothrow) Packet(copyData, packetSize, 0, clientID, 0, instanceID, 0);
...
double NetModeTcpPrefixSize::GetPartialPacketPercentage() const
{
	...
	if(partialPacket.GetUsedSize() >= Packet::prefixSizeBytes)
	{
		double packetFinalSize = static_cast<double>(partialPacket.GetPrefixSizeT());
		double packetCurrentSize = static_cast<double>(partialPacket.GetUsedSize() - Packet::prefixSizeBytes);
...
bool NetModeTcpPrefixSize::TestClass()
{
	...
	if(obj.GetPartialPacketCurrentSize() != amountToAdd || obj.GetPacketAmount() != 0)
	{
	...
	buffer.len = remainingData;
	obj.DealWithData(buffer,remainingData,NULL,1,2);

	if(obj.GetPartialPacketCurrentSize() != 0 || obj.GetPacketAmount() != 1)
	{
		cout << "DealWithData is bad\n";
		problem = true;
	}
	...
	if(obj.GetPartialPacketCurrentSize() != 0 || obj.GetPacketAmount() != 3)
	{
		cout << "DealWithData is bad\n";
		problem = true;
	}
...
/**
 * @brief Techlord Modification for TCP_FOR_HTTP Mode. Enable/Disable the prefix in use, which indicates the end of a packet.
 *
 * @return the prefix in use.
 */
void NetModeTcpPrefixSize::SetAffixEnable(bool enabledflag)
{
...
/**
 * @brief Techlord Modification for TCP_FOR_HTTP Mode. Retrieves the prefix enabledflag.
 *
 * @return the prefix in use.
 */
bool NetModeTcpPrefixSize::GetAffixEnable()
{
 ...
 * @return the prefix in use.
 */
void NetModeTcpPrefixSize::SetBinaryMode(bool enabledflag)
{
 ...
 * @return the prefix in use.
 */
bool NetModeTcpPrefixSize::GetBinaryMode()
{
 ...
 * @return the prefix in use.
 */
void NetModeTcpPrefixSize::SetHeaderFound(bool foundflag)
{
 ...
 * @return the prefix in use.
 */
bool NetModeTcpPrefixSize::GetHeaderFound()
{
