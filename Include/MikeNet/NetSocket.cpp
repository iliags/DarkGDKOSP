 ...
 *
 * @param [in]	sendObject	Object to send. 
...
NetUtility::SendStatus NetSocket::Send(NetSend & sendObject, const NetAddress * sendToAddr, unsigned int timeout)
{
	...
	if(sendToAddr == NULL)
	{
		iResult = WSASend(winsockSocket,sendObject.GetBuffer(),sendObject.GetBufferAmount(),&sendObject.bytes,NULL,&sendObject.overlapped,NULL);
	}
	// Send to address not connected to
	else
	{
		iResult = WSASendTo(winsockSocket,sendObject.GetBuffer(),sendObject.GetBufferAmount(),&sendObject.bytes,NULL,(SOCKADDR*)sendToAddr->GetAddrPtr(),sizeof(SOCKADDR),&sendObject.overlapped,NULL);	
	}
