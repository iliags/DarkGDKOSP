 ...
 */
bool NetSocketUDP::GetModeLoaded() const
{
...
{
	_ErrorException((GetModeLoaded() == false),"performing a UDP operation, UDP mode is not loaded and must be",0,line,file);
}
...
{
	_ErrorException((GetModeLoaded() == true),"loading a UDP mode, UDP is already loaded and cannot be changed during run time",0,__LINE__,__FILE__);
