 ...
 */
void NetInstanceClient::Initialize(const EncryptKey * p_decryptKey)
{
 ...
 */
NetInstanceClient::NetInstanceClient(NetSocketTCP * p_socketTCP, NetSocketUDP * p_socketUDP, bool p_handshakeEnabled, unsigned int p_sendTimeout, const EncryptKey * p_decryptKey, size_t p_instanceID) :
		...
		)
{
	...
	Initialize(p_profile.GetDecryptKeyUDP());
}
...
NetUtility::ConnectionStatus NetInstanceClient::ThreadHandshakeRoutine()
{
	try
	{
		Timer connectionTimeout(GetConnectTimeout());
		...
		while(true)
		{
			...
			if(packets > 0)
			{
				...
				if(IsEnabledUDP() == true)
				{
					...
					NetModeUdp * modeUDP = NetModeUdp::GenerateModeUDP(mode,maxClients.Get(),numOperations,recvSizeUDP,decryptKey);
...
NetUtility::ConnectionStatus NetInstanceClient::Connect(const NetAddress * addressTCP, const NetAddress * addressUDP, size_t connectionTimeout, bool block)
{
	...
	handshakeThread = new (nothrow) ThreadSingle(&ThreadHandshakeRoutineStatic,this,false);
...
bool NetInstanceClient::TestClass()
{
	...
	profile.SetModeTCP(NetMode::TCP_POSTFIX);
