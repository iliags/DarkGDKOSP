...
/** @brief Constructor. */
NetModeUdpCatchAllNo::NetModeUdpCatchAllNo(size_t numClients) : NetModeUdpCatchAll(numClients)
{
...
counterJustResetSoTryAgain:
	...
	if(newPacketCounter != 0)
	{
		if(newPacketCounter >= recvCounter[clientID].Get())
		{
			...
			Packet * newPacket = new (nothrow) Packet(buffer,completionBytes-packetBuffer.GetCursor(),packetBuffer.GetCursor(),clientID,0,instanceID,newPacketCounter);
