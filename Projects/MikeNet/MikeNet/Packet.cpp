...
void Packet::DoEncryptionOperation(bool encryption, const EncryptKey & key, bool block)
{
	...
	SetupThreadsLocal(ThreadSingle::GetNumLogicalCores(),&EncryptionThread,NULL);
...
void Packet::Erase(size_t startPos, size_t amount)
{
	...
	try
	{
		...
		if(amount > 0)
		{
			...
			// Change cursor position, ensuring that it doesn't go negative (which is impossible)
			// Large type is used so that negative value can be detected
			__int64 newCursorPos = static_cast<long long int>(cursorPos) - static_cast<long long int>(amount);
			if(newCursorPos > 0)
			{
				cursorPos = static_cast<size_t>(newCursorPos); // Note: no data can be lost
			}
			else
			{
				cursorPos = 0;
...
void Packet::AddStringC(const char * add, size_t length, bool prefix)
{
	...
	try
	{
		...
		// Move cursor back to original position so that we can inset data correctly
...
{
	DefaultVariables(true); // ensures that equalWSABUF works properly
...
void Packet::LoadFull(const WSABUF & paraData, size_t paraUsed, size_t paraOffset, size_t paraClientFrom, size_t paraOperation, size_t paraInstance, clock_t paraClock)
{
	...
	{
		DefaultVariables(true);
		equalWSABUF(paraData,paraUsed - paraOffset,paraOffset);
