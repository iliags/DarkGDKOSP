...
StoreVector<SoundInstanceOutput*> SoundInstanceOutput::instanceOutputList;
 ...
 */
SoundInstanceOutput::SoundInstanceOutput(size_t msStoredDataCap, size_t timeGapCap, const SoundFormat & format, unsigned int deviceID)
	: bufferInUseCount((size_t)0), outputDataLastAdded(0)
{
	...
	instanceOutputList.Enter();
		...
		{
			instanceOutputThread = new (nothrow) ThreadSingle(&_ManageSoundOutput,NULL,false);
			Utility::DynamicAllocCheck(instanceOutputThread,__LINE__,__FILE__);
		}

		SoundInstanceOutput ** ptr = new (nothrow) SoundInstanceOutput*(this);
		Utility::DynamicAllocCheck(ptr,__LINE__,__FILE__);
		instanceOutputList.Add(ptr);
...
SoundInstanceOutput::~SoundInstanceOutput()
{
	...
	try
	{
		...
		instanceOutputList.Enter();
			size_t position = 0;
			SoundInstanceOutput * findMe = this;
			bool found = instanceOutputList.Find(findMe,&position);
			_ErrorException((found == false),"removing a sound output instance from the output list, instance is not in list",0,__LINE__,__FILE__);
			instanceOutputList.Erase(position);
...
size_t SoundInstanceOutput::DequeuePacket()
{
	...
	buffer->dwBufferLength = packet.GetUsedSize();
...
void SoundInstanceOutput::SetHardwarePlaybackRate(double rate)
{
	...
	size_t convertedRate = MAKELONG(fraction,integer);
...
void SoundInstanceOutput::SetHardwarePitch(double pitch)
{
	...
	size_t convertedPitch = MAKELONG(fraction,integer);
...
{
	// Convert rate into waveform format
	size_t convertedVolume = MAKELONG(left,right);
 ...
 * to the queue, before ending the wait and resuming playback regardless of the amount of data stored.
 */
void SoundInstanceOutput::SetSmoothValues( size_t msStoredDataCap, size_t timeGapCap )
{
	outputData.Enter();
